...
class Model:
    ...
    '''
    BuildAndSolveNEFF builds and solves for effective index.
    '''

    def SolveEffectiveIndex(self):
        self.Variables = self.structure.Variables
    ...
    def PlotMode(self):
        ...
        epsFile = self.Variables['workingDir']+ subdir + self.Variables['MPsimname'] + "-eps-000000.00.h5"
        eyFile      = self.Variables['workingDir'] + subdir + self.Variables['MPsimname'] + "-ey-000000.00.h5"
        ...
        # first just save a png of the fields and structure

        fig,ax = plt.subplots(dpi=200)

        ax.imshow(epsfmod,cmap="Greys")
        plt.axis('off')
        
        plt.savefig(self.Variables['workingDir']+subdir+"eps.png")
        plt.close()
        ...
        plt.savefig(self.Variables['workingDir']+subdir+"fields.png")
        plt.close()
        ...
        plt.savefig(self.Variables['workingDir']+subdir+"ModeProfile.pdf")
        #plt.show()
        plt.close()
    ...
    def saveMetaData(self):
        ...
        with open(self.Variables['workingDir'] + subdir + 'metadata.json', 'w') as file:
